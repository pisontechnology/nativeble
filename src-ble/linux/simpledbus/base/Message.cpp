...
Message::Message(DBusMessage* msg) : _msg(msg), iter_initialized(false), indent(0) {}
...
bool Message::is_valid() { return _msg != nullptr; }

std::string Message::get_signature() {
    if (is_valid() && iter_initialized) {
...
std::string Message::to_string() {
    ...
    oss << type_to_name(dbus_message_get_type(_msg)) << "[" << sender << "->" << destination << "] ";
...
Holder Message::extract() {
    ...
    Holder holder = _extract_generic(&_iter);
    return holder;
}
...
bool Message::extract_has_next() { return iter_initialized && dbus_message_iter_has_next(&_iter); }
